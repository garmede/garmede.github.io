...
> 꼭 하나의 셰이더를 다양하게 써야 최적화가 되는 것은 아닙니다. 최적화는 상황에 따라 달라지기 십상입니다. 하나의 셰이더를 다양하게 쓰기 위해 `if`문이나 베리언츠로 덩치가 커지기 시작하면 그것도 문제가 될 수 있습니다. 셰이더가 너무 무거워져서 프레임이 안 나오거나, 베리언츠 개수가 너무 많아져서 프로그램이 감당하지 못하고 터지기도 합니다.🤯
{: .prompt-info }
...
> 유니티의 `.shader`파일에서 실제 셰이더 부분은 일반적으로 `CGPROGRAM`에서 부터 `ENDCG` 사이의 내용입니다. 혹은 사용하는 언어나 목적에 따라 `HLSLPROGRAM`...`ENDHLSL`이거나 `GLSLPROGRAM`...`ENDGLSL`일 수도 있습니다.
> 즉, 그 밖의 내용들은 *셰이더가 아니라는 뜻*입니다. 셰이더가 아닌 부분은 [ShaderLab](https://docs.unity3d.com/kr/current/Manual/SL-Reference.html)이라고 부르는 유니티에서 셰이더를 관리하기 위한 스크립트입니다.
{: .prompt-info }
...
- **유형**: 이 프로퍼티가 어떤 유형인지 여기에 적은 대로 유니티에 노출됩니다.
